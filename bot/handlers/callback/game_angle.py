...
from config import ANGLE_ROUTE, HOST, INVALIDATE_STATE_MINUTES, PROXY_PREFIX
from utils.security import generate_user_secret


async def callback_game_angle(query: types.CallbackQuery, captcha_storage: RedisStorage, captcha_state: FSMContext, bot: Bot):
    ...
    await query.answer(url=f"{HOST}{PROXY_PREFIX}{ANGLE_ROUTE}?user_id={user.id}&first_name={quote(user.first_name)}&public_key={user_data['secret']['public_key']}",)
